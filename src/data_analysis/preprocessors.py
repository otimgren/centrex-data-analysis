...
class NormalizedAbsorption(PreProcessor):
    ...
    background_slice: np.s_ = np.s_[
        -3000:
    ]  # Slice used for determining absorption signal
    # background
    ...
    def apply_normalization(self, df: pd.DataFrame) -> None:
        ...
        df.NormalizedAbsorption = df.NormalizedAbsorption / (
            df.NormalizedAbsorption.apply(lambda x: x[self.background_slice].mean())
        )
...
class IntegratedAbsorption(PreProcessor):
    ...
    integration_slice: np.s_ = np.s_[
        10:2000
    ]  # Slice for calculating integral of absorption
    background_slice: np.s_ = np.s_[
        -3000:
    ]  # Slice used for determining absorption signal
    # background
    ...
    def process_data(self, df: pd.DataFrame) -> None:
        df["IntegratedAbsorption"] = df.NormalizedAbsorption.apply(
            self.calculate_integrated_absorption
        )
    ...
    def calculate_integrated_absorption(self, trace: np.ndarray) -> float:
        ...
        """
        return -np.trapz(
            trace[self.integration_slice] - np.mean(trace[self.background_slice])
        )
...
class AbsorptionBigEnough(PreProcessor):
    ...
    absorption_cutoff: float = 5.0  # Integrated absorption needs to be bigger than this to enter
...
class MicrowavesON(PreProcessor):
    ...
    def process_data(self, df: pd.DataFrame) -> None:
        df["MicrowavesON"] = (
            df.MicrowaveShutter.apply(np.max) > self.shutter_open_cutoff
        )
