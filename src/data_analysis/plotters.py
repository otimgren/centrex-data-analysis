...
class GaussianFitPlotter(PlotterDefault):
    ...
    def plot_fit(self, result: lmfit.model.ModelResult) -> None:
        ...
        self.ax.contour(result.eval(x = x_plot, y = y_plot).reshape(self.image_shape),
                        origin = 'lower')
    ...
    def plot_fit_maximum(self, result: lmfit.model.ModelResult) -> None:
        ...
        self.ax.plot(self.center_x, self.center_y, color = 'red', marker = 'x', markersize = 10)

    def plot_cut_along_vertical(self, image: np.ndarray) -> None:
        """
        Plot the data along a vertical line through the maximum
        """
        self.right_ax.plot(image[:,int(self.center_x)],np.arange(0,self.image_shape[1]))
    ...
    def plot_cut_along_horizontal(self, image: np.ndarray) -> None:
        ...
        Plot the data along a vertical line through the maximum
        """
        self.top_ax.plot(np.arange(0,self.image_shape[0]),image[int(self.center_y),:])
...
class SwitchingParamScanPlotter(Plotter):
    ...
    def plot(self, df: pd.DataFrame, param_name: str, signal_name: str, switch_name: str) -> None:
        fig, ax = plt.subplots(figsize = (16,9))
        ax.errorbar(df[param_name], df[signal_name+'_ON'], marker = 'x', label = switch_name)
        ax.errorbar(df[param_name], df[signal_name+'_OFF'], marker = 'x', label = 'NOT '+switch_name)
        ax.set_xlabel(param_name, fontsize = 16)
        ax.set_ylabel(signal_name, fontsize = 16)
        ax.legend()
